...
read_user_input (enum game_state* state)
{
    ...
    if ((ch = getch ()) > 0)
    {
        switch (ch)
        {
            ...
            case 'w': case KEY_UP:    set_worm_heading (WORM_UP);    break;
            case 's': case KEY_DOWN:  set_worm_heading (WORM_DOWN);  break;
            case 'a': case KEY_LEFT:  set_worm_heading (WORM_LEFT);  break;
            case 'd': case KEY_RIGHT: set_worm_heading (WORM_RIGHT); break;
            case 'u':                 set_worm_heading (WORM_UL);    break;
            case 'i':                 set_worm_heading (WORM_UR);    break;
            case 'o':                 set_worm_heading (WORM_DR);    break;
            case 'p':                 set_worm_heading (WORM_DL);    break;
            case 'b': nodelay (stdscr, FALSE); break; // single step
...
set_worm_heading (enum worm_heading dir)
{
    switch (dir)
    {
        ...
        case WORM_UL:    worm_dx = -1; worm_dy = -1; break;
        case WORM_UR:    worm_dx =  1; worm_dy = -1; break;
        case WORM_DL:    worm_dx = -1; worm_dy =  1; break;
        case WORM_DR:    worm_dx =  1; worm_dy =  1; break;
    }
}
